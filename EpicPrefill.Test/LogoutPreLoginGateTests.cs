...
namespace EpicPrefill.Test
{
    ...
    /// so a logout sent to a container still mid-OAuth (_isLoggedIn == false) was rejected by the gate
    /// before HandleLogoutAsync (itself unconditionally safe) ever ran. SocketCommandInterface has no
    /// public seam (it owns a live SocketServer, never bound here since StartAsync is never called), so
    /// this drives the private HandleCommandAsync gate via reflection.
    /// </summary>
    ...
    public sealed class LogoutPreLoginGateTests : IDisposable
    {
        ...
        public async Task Logout_WhileNotLoggedIn_IsNotRejectedByPreLoginGate()
        {
            using var socketInterface = new SocketCommandInterface(
                Path.Combine(Path.GetTempPath(), $"epic-prelogin-gate-{Guid.NewGuid():N}.sock"));
