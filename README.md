...
[![Platform: Epic Games](https://img.shields.io/badge/Epic%20Games-2a2a2a?style=for-the-badge&logo=epicgames&logoColor=white)](https://store.epicgames.com/)
...
Epic Games prefill daemon for [LANCache](https://lancache.net/) — a companion to
[**LANCache Manager**](https://github.com/regix1/lancache-manager), which
coordinates the prefill providers.

It downloads your owned Epic Games content through your lancache *before* you
...
- **Account login** — sign in with your Epic Games account via Epic's OAuth flow.
...
> A prebuilt container image is published at `ghcr.io/regix1/epic-prefill-daemon`
...
1. **Authenticate** — log in with your Epic Games account via Epic's OAuth flow.
2. **Select titles** — choose which owned apps to prefill from an interactive menu.
3. **Resolve** — each app's manifest and CDN URLs are looked up from Epic's content system.
4. **Prefill** — each app's content is fetched through the lancache and discarded, warming the cache.

## Requirements

- A running [LANCache](https://lancache.net/) with the **`epicgames`** cache-domain
